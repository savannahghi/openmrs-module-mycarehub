 ...
 * extremes For example, the mood options on myCareHub are: VERY_HAPPY, HAPPY, MEH, SAD, VERY_SAD
 * Aside from the mood, the client also enters a `Note` which describes their mood If a client has
 * selected MEH, SAD or VERY_SAD as their mood, a service request is triggered by the myCareHub
 * platform and the service request is synchronized to KenyaEMR via the myCareHub module Health
 * Diary entries are recorded once a day (every 24hrs)
 */
