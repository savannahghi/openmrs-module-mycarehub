...
public class HibernateMyCareHubPatientDao implements MyCareHubPatientDao {
  ...
  @Override
  public List<ConsentedPatient> getConsentedPatientsInList(List<Integer> patientIdList) {
    return getSession()
        .createCriteria(ConsentedPatient.class)
        .add(Restrictions.in("patientId", patientIdList))
        .list();
  }

  @Override
  public void saveNewConsentedPatients(ConsentedPatient consentedPatient) {
    getSession().save(consentedPatient);
  }
        ...
        sessionFactory
            ...
            .createSQLQuery(
                "SELECT patient.patient_id as patientId FROM patient "
                    ...
                    + "WHERE uuid = :cccIdentifierTypeUuid) "
  ...
  private List<Integer> getTestsConceptsList() {
    SQLQuery testsConcepts =
        getSession()
            .createSQLQuery("SELECT concept_id FROM concept WHERE class_id = 1 AND retired=0");
