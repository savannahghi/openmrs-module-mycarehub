...
public class RedFlagServiceImplTest {
	...
	public void getPagedRedFlagsByRequestType() {
		...
		when(myCareHubRedFlagDao.getPagedRedFlagsByRequestType("TEST", redFlagsList.get(0).getRequestType(), 1, 3))
		        .thenReturn(redFlagsList);
		
		List<RedFlags> redFlags = fakeRedFlagServiceImpl.getPagedRedFlagsByRequestType("TEST", redFlagsList.get(0)
		        .getRequestType(), 1, 3);
		assertEquals(1, redFlags.size());
		assertNotNull(redFlags);
	}
